...
- D10：接入结局报告与本地存档闭环，Day 7 成功后进入 `ending` 报告页，按清醒值生成“替罪羊 / 举报者”评级分支，支持分享文案复制、PNG 报告图导出、二维码视觉、存档读取、自动保存和清除存档。
...
- D7 内容对齐补充：根据最新飞书“文本配置”更新 Week 1 垂直切片验收前已有内容口径，升级已存在的数据卡、用户、新闻、协议词、清洗图标、舆论题、画像拼图、谈判话术、协议扫描模板、证据链模板、结局报告模板、每日独白和黑盒台词；新增同步脚本。完整 UI 文案仍作为后续开发依据。
